# apply-fixes-v10.ps1 (SELF-CONTAINED - includes everything, not just diffs)
#
# NEW:
# - pricing-engine.js: TOS score, buy/sell price recommendations, slot-
#   pressure curve (soft 40-90, steep 90-97, blocked 97-100), relist logic
#   that RE-RUNS with fresh data instead of a fixed discount.
# - positions-store.js: tracks individual bought-card instances
#   (positionId), separate from cardId (card TYPE). One cardId can have
#   several open positions at once.
# - New CLI commands (human-in-the-loop, one short command per real-world
#   action): buy, list, sold, expired, relist, positions (dashboard).
#
# FIXED:
# - marketLiquidityRatio now computed from ONE source (FUTBIN sales-history
#   table gives both sales/hr AND listings/hr from the same data) instead
#   of mixing FUTBIN sales with FUT.GG's Live Auctions snapshot.
# - FUT.GG Live Auctions kept as a separate 'activeAuctionsSnapshot' field -
#   advisory only, never used as an engine input.
#
# All 9 files passed `node --check` + a logic smoke-test before bundling.
...
Write-Host "Writing v10 files (self-contained)..." -ForegroundColor Cyan
...
Write-Host "  Optioneel: zet ACCOUNT_BUDGET en RISK_PROFILE in .env" -ForegroundColor Yellow
Write-Host "  (RISK_PROFILE=conservative|standard|aggressive, default: standard)" -ForegroundColor Yellow
Write-Host ""
$doCommit = Read-Host "Commit + push these changes to git now? (y/n)"
if ($doCommit -eq "y") {
  Push-Location ..
  .\scripts\git-checkpoint.ps1 -Message "feat: pricing-engine.js + position tracking + human-in-the-loop transaction CLI"
  Pop-Location
} else {
