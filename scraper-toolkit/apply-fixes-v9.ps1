# apply-fixes-v9.ps1
# - NEW: FUTBIN sales-history scraper (dedicated /sales/ page) with real
#   bid/bin/expired classification per transaction (via confirmed icon
#   selectors) - gives a REAL sales-per-hour estimate instead of a guess.
# - FIXED: FUT.GG Recent Sales / Live Auctions extraction - was disabled
#   in v6 (returned noise), now uses confirmed <h3>-heading + <table>
#   structure for clean structured rows.
# - NEW: liquidityRatio (sales/listings), bidBinBreakdown in merged output
#   and the pretty overview - closes the listings_per_hour data gap from
#   the pricing strategy report.
# All 4 changed .js files passed `node --check` before bundling.
...
Write-Host "Writing v9 files..." -ForegroundColor Cyan
...
Write-Host "  Note: this scrape will take longer now (FUTBIN does 2 page loads" -ForegroundColor Yellow
Write-Host "  instead of 1 - market page + sales history page, in parallel with" -ForegroundColor Yellow
Write-Host "  the other sources, so total time still driven by the slowest one)" -ForegroundColor Yellow
...
if ($doCommit -eq "y") {
  Push-Location ..
  .\scripts\git-checkpoint.ps1 -Message "feat: real FUTBIN sales history (bid/bin/expired) + FUT.GG Recent Sales/Live Auctions - closes listings_per_hour data gap"
  Pop-Location
} else {
